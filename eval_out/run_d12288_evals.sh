#!/usr/bin/env bash
# Full eval suite for the d12288 dpc run (latent_dim 12288, finished ep50), against the
# d6144 dpc run at the SAME epoch (both step_0014200.pt) — the 4x point of the width series (d3072 results: eval_out/*d3072_50*).
#
# DO NOT use d12288/best_val.pt: on d3072 it was epoch 10, pre-clustering, centroids never
...
cq() {     # ~25 min, same 1M sample as cq_init_arms.json / cq_tanh50.json
    uv run python -u -m geoae.interp.clustering_quality \
        --checkpoints $D12288 $D6144 --names d12288 d6144 \
...
step "concept probe"        "[ -f eval_out/probe_d12288_50.json ]"                                     probe
step "geometry"             "[ -d figures/d12288_50/tsne ]"                                            geom
step "clustering quality"   "[ -f eval_out/cq_d12288_50.json ]"                                        cq
step "DB14 steering"        "[ -f results/steer_db14_d12288_50.json ]"                                 steer
step "range DB14 (d')"      "done_range results/range_intervention_db14_d12288_50_dprime.json"         range_db14
step "range bias_in_bios"   "done_range results/range_intervention_biasbios_d12288_50_dprime.json"     range_bios
